...
class ViewController: UIViewController {
    ...
    func gameState(){
        if(currentGame.movesRemain <= 0){
            currentGame.guessAll();
            updateUI();
            losses += 1;
    ...
    func updateUI(){
        CorrectWordLabel.text = currentGame.formatWord;
        ScoreLabel.text = "Wins: \(wins), Losses: \(losses)";
        tree.image = UIImage(named: "Tree \(currentGame.movesRemain)-1");
    }
    ...
    //Restart the round
    func newRound(){
        let randomIndex = random(words.count);
        if(words.isEmpty){
            for button in InputButtons{button.isEnabled = false;}
            return;
        }
        let newWord = words[randomIndex];
        words.remove(at: randomIndex);
        currentGame = Game(word: newWord, movesRemain: incorrect, guessed: []);
